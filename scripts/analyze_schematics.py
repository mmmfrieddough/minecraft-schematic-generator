...
def list_subdirectories_with_file_counts(directory):
    ...
    all_dirs = [os.path.join(root, dir) for root, dirs,
                files in os.walk(directory) for dir in dirs]
    for subdir_path in tqdm(all_dirs, desc="Processing directories"):
        file_count = len([file for file in os.listdir(
            subdir_path) if os.path.isfile(os.path.join(subdir_path, file))])
...
def plot_subdirectory_file_counts(subdir_file_counts):
    ...
    sorted_subdirs = sorted(subdir_file_counts.items(),
